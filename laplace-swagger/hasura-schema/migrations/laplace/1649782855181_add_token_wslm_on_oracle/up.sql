CREATE OR REPLACE VIEW "laplace"."oracle_token_price" AS 
(SELECT
    ...
    (
      ...
      WHERE
        (
          ...
          AND (
            (oracle_instant_swp.contract_name) :: text = 'simple-weight-pool-alex' :: text
          )
          AND (
            oracle_instant_swp.block_height = (
              SELECT
                max(oracle_instant_swp_1.block_height) AS max
              FROM
    ...
    (
      ...
      WHERE
        (
          ...
          AND (
            (oracle_instant_swp.token_y) :: text = 'token-wslm' :: text
          )
          AND (
            (oracle_instant_swp.contract_name) :: text = 'simple-weight-pool-alex' :: text
          )

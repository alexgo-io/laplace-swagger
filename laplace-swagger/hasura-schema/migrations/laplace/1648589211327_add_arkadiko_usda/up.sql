...
WITH coin_geiko AS ( SELECT
    CASE
      WHEN ((coin_gecko.token) :: text = 'token-usda' :: text) THEN 'token-wusda' :: character varying
      WHEN ((coin_gecko.token) :: text = 'token-mia' :: text) THEN 'token-wmia' :: character varying
      WHEN ((coin_gecko.token) :: text = 'token-diko' :: text) THEN 'token-wdiko' :: character varying
...
usda AS (
  SELECT
  case WHEN ((external_token_price.token_name) :: text = 'USDA' :: text) THEN 'token-usda' :: character varying
    else token_name end as token,
    COALESCE(
